...
def cropImages(imgList, tempFolderPath, upperArchImage, lowerArchImage, pid):
    try:
        print(f'Cropping images in {tempFolderPath}\croppedImages')
        count = 0
        clinicType = int(pid[6])
        if(os.path.exists(upperArchImage) and os.path.exists(lowerArchImage) and clinicType != 5 and clinicType != 2 and (logoRequired=='Y' or logoRequired=='y')):
            shutil.copyfile(upperArchImage, fr'{tempFolderPath}\extractedImages\Image60.png')
            shutil.copyfile(lowerArchImage, fr'{tempFolderPath}\extractedImages\Image61.png')
        for i in imgList:
...
def editPDF(tempFolderPath, tpr, ipr, pid, name, operator, clinicId, wisealignClinic):
    try:
        ...
        pdf2 = tpr
        pdf3 = fr'{tempFolderPath}\splitPDF\OrthoReport2.pdf'
        pdf4 = fr'{tempFolderPath}\splitPDF\OrthoReport3.pdf'
        pdf5 = fr'{tempFolderPath}\splitPDF\OrthoReport4.pdf'
        pdf6 = fr'{tempFolderPath}\splitPDF\OrthoReport5.pdf'
        pdf7 = ipr
        pdf8 = fr'{tempFolderPath}\splitPDF\OrthoReport6.pdf'
        ...
        if(clinicType != 5 and clinicType != 2 and (logoRequired=='Y' or logoRequired=='y')):
            pdf9 = fr'{tempFolderPath}\splitPDF\OrthoReport0.pdf'
            pdfFilesList.append(pdf9)
        count = 0
        for item in pdfFilesList:
            ...
            if(wisealignClinic!='79' or (logoRequired=='N' or logoRequired=='n')):
                ...
                footer = os.path.join(sys.path[0], fr'Resources\Images\footer.jpg')
                header1 = os.path.join(sys.path[0], fr'Resources\Images\header1.png')
                header2 = os.path.join(sys.path[0], fr'Resources\Images\header2.png')
                smile = os.path.join(sys.path[0], fr'Resources\Images\smile.jpg')
                circle = os.path.join(sys.path[0], fr'Resources\Images\circle.png')
                circle1 = os.path.join(sys.path[0], fr'Resources\Images\circle1.png')
                ...
                if(item == pdf7):
                    ...
                    if(clinicType != 5 and clinicType != 2 and (logoRequired=='Y' or logoRequired=='y')):
                        can.drawImage(logo, 420, 775, 160, 50, preserveAspectRatio=True, mask='auto')
                        can.drawImage(header2, 0, 762, 400, 80, mask='auto')
                    ...
                    if(clinicType != 5 and clinicType != 2 and (logoRequired=='Y' or logoRequired=='y')):
                        can.drawImage(logo, 420, 775, 160, 50, preserveAspectRatio=True, mask='auto')
                        if(item == pdf2):
                            can.drawImage(header1, 0, 762, 400, 80, mask='auto')
                        else:
                            can.drawImage(header2, 0, 762, 400, 80, mask='auto')
                    elif(clinicType == 5 and (logoRequired=='Y' or logoRequired=='y')):
                ...
                if(clinicType != 5 and clinicType != 2 and (logoRequired=='Y' or logoRequired=='y')):
                    can.drawImage(footer, 0, 0, 600, 30, mask='auto')
                    can.drawImage(circle1, 6, 5, 20, 20, preserveAspectRatio=True, mask='auto')
                else:
                ...
                if(clinicType != 5 and clinicType != 2 and (logoRequired=='Y' or logoRequired=='y')):
                    ...
                    can.drawString(115, 10, 'www.rejove32aligners.com | info@rejove32aligners.com')
                    can.setFillColor(HexColor('#000000'))
                    can.setFont('MuseoSansRounded', 11)
                    
                elif(clinicType == 2 and (logoRequired=='Y' or logoRequired=='y')):
                    can.setFont('MuseoSansRounded', 14)
                    can.drawString(130, 10, 'https://my6senses.com/ | info@my6senses.com | 18602580123')
                ...
                if(clinicType != 5 and clinicType != 2 and (logoRequired=='Y' or logoRequired=='y')):
                    ...
                    can.setFillColor(HexColor('#FFFFFF'))
                    if(item == pdf2):
                        can.drawString(40, 805, 'Treatment')
                        can.drawString(15, 785, 'Planning Report')
                        can.setFont('MuseoSansRounded', 12)
                        can.drawString(200, 815, 'Your customized treatment plan')
                        can.drawString(200, 800, 'towards achieving your dazzling')
                        can.drawString(200, 785, 'smile is ready. Lets explore')
                    can.setFillColor(HexColor('#FFFFFF'))
                    if(item == pdf5 or item == pdf6):
                        can.drawString(120, 805, 'Comparison of')
                        can.drawString(70, 785, 'Pre & Post Treatment Views')
                    if(item == pdf7):
                        can.drawString(120, 795, 'IPR Sheet')
                    if(item == pdf8):
                        can.drawString(70, 795, 'Teeth Movement Report')
                    if(item == pdf9):
                        can.drawString(100, 805, 'Superimposition of')
                        can.drawString(50, 785, 'Pre-Treatment & Post-Treatment')   
                        can.setFillColor(HexColor('#000000')) 
                        can.setFont('MuseoSansRounded', 12)
                        can.drawString(270, 730, 'Upper View')
                        can.drawString(270, 440, 'Lower View')
                    can.setFont('MuseoSansRounded', 11)
                    can.setFillColor(HexColor('#000000'))
                    if(item == pdf2):
                        can.drawString(12, 10, '1')
                        can.drawString(95, 40, 'Doc. No.: R320TPR-01   |   Supersedes: R320TPR-00   |   Effective Date: 08-01-2024')
                    if(item == pdf9):
                        can.drawString(12, 10, '2')
                        can.drawString(110, 40, 'Doc. No.: R320SPPT-00   |   Supersedes: Nil   |   Effective Date: 08-01-2024')
                    if(item == pdf5):
                        can.drawString(12, 10, '3')
                        can.drawString(110, 40, 'Doc. No.: R320CPPT-00   |   Supersedes: Nil   |   Effective Date: 08-01-2024')
                    if(item == pdf6):
                        can.drawString(12, 10, '4')
                        can.drawString(110, 40, 'Doc. No.: R320CPPT-00   | Supersedes: Nil   |   Effective Date: 08-01-2024')
                    if(item == pdf7):
                        can.drawString(12, 10, '5')
                        can.drawString(50, 40, 'Doc. No.: RSP/F/QA/24/05-02   |   Supersedes: RSP/F/QA/24/05-01   |   Effective Date: 08-01-2024')
                    if(item == pdf8):
                        can.drawString(12, 10, '6')
                        can.drawString(110, 40, 'Doc. No.: R320TMR-00   |   Supersedes: Nil   |   Effective Date: 08-01-2024')
                else:        
                    if(item == pdf1):
...
def mergePDFs(tempFolderPath, caseName, finalReportPath, wisealignClinic, pid):
    try:
        ...
        pdf9 = fr'{tempFolderPath}\splitPDF\OrthoReports8.pdf'
        if(wisealignClinic=='79' and (logoRequired=='Y' or logoRequired=='y')):
            pdfFiles = [pdf1, pdf2, pdf5, pdf6, pdf7]
        elif(clinicType != 5 and clinicType != 2 and (logoRequired=='Y' or logoRequired=='y')):
            pdfFiles = [pdf2, pdf9, pdf5, pdf6, pdf7, pdf8]
        else:
...
for caseNumber in range(0,sourceFolderTotalDir):
    ...
    if os.path.isdir(cases[caseNumber]):
        ...
        upperArchImage = cases[caseNumber]+fr"\UPPER.png"
        lowerArchImage = cases[caseNumber]+fr"\LOWER.png"
    ...
    if(clinicType != 5 and clinicType != 2 and (logoRequired=='Y' or logoRequired=='y')):
        imgList = [31, 32, 37, 38, 43, 44, 49, 50, 55, 56, 60, 61]
    else:
        imgList = [31, 32, 37, 38, 43, 44, 49, 50, 55, 56]

    cropImages(imgList, tempFolderPath, upperArchImage, lowerArchImage, pid)
    ...
    <body>
        <div>
            ...
            <table cellpadding='1' cellspacing='1' class='superimpositionSection'>
                <tr>
                    <td class='center'>
                        <img src="''' + f'{tempFolderPath}\croppedImages\Image60.png' + '''" style='height:400px; width:600px;' class='orthoImage' alt='No Image'>
                    </td>
                </tr>
            </table>
			<br/><br/><br/><br/>
            <table cellpadding='1' cellspacing='1' class='superimpositionSection'>
                <tr>
                    <td class='center'>
                        <img src="''' + f'{tempFolderPath}\croppedImages\Image61.png' + '''" style='height:400px; width:600px;' class='orthoImage' alt='No Image'>
                    </td>
                </tr>
            </table>
			<br/><br/><br/>
            <table cellpadding='1' cellspacing='1' class='superimpositionSection'>
                <tr>
                    <td class='center'>
                        <h3>Amount of tooth movement achieved at the end of the treatment</h3>
                    </td>
                </tr>
				<tr>
					<td class='center'>
                        <h3>
                            <img src="''' + os.path.join(sys.path[0], fr'Resources\Images\box1.png') + '''" style='height:25px; width:25px;' class='orthoImage' alt='No Image'> 
                            &nbsp; Blue: Post-Treatment
                        </h3>
                        <h3>
                            <img src="''' + os.path.join(sys.path[0], fr'Resources\Images\box2.png') + '''" style='height:25px; width:25px;' class='orthoImage' alt='No Image'> 
                            &nbsp; White: Pre-Treatment 
                        </h3>
                    </td>
                </tr>
            ...
            <table cellpadding='1' cellspacing='1' style='margin-top:-50px;' class='scannedImagesSection'>
                ...
                <tr>  
                    <td class='center'><img src="''' + f'{tempFolderPath}\extractedObject\Image0.png' + '''"
                            class='orthoImage1' alt='No Image'></td>
    ...
    if(int(pid[6]) != 2 or (logoRequired=='N' or logoRequired=='n')):
           htmlContent = htmlContent + '''<p style='font-size:28px;color:#0F6579;line-height:150%;'>Do reach out to our team if you have any further questions and we’d be happy to have our executives speak to you. Have a great day, and remember, <span style='color:#0F6579;'> keep smiling.</span><img src="''' + os.path.join(sys.path[0], fr'Resources\Images\smily.png') + '''"></p>
            <br/>'''
    ...
    if(int(pid[6]) != 5 and int(pid[6]) != 2 and (logoRequired=='Y' or logoRequired=='y')):
        htmlContent = htmlContent + '''<p style='font-size:28px;color:#D62460;line-height:150%;margin-bottom:0px;padding-bottom:0px;'>Best Regards <br/> (<b>Team Rejove32 Aligners</b>)</p>'''
    ...
    mergePDFs(tempFolderPath, caseName, finalReportPath, wisealignClinic, pid)
    ...
    if(os.path.exists(fr'{sourceFolderPath}\{caseName}\Videos')==True):
        ...
        addProgressbarToVideo(tempFolderPath, pid, logoRequired)
