...
def createVideoThumbnail(tempFolderPath, caseName, pid, aligners, operator):
    try:
...
def imagesToVideo(tempFolderPath, multiple):
    try:
        ...
        if(multiple):
            ...
            fr'{tempFolderPath}\croppedVideos\upload plan.avi.jpg',
	        fr'{tempFolderPath}\croppedVideos\upload plan 001.avi.jpg',
	        fr'{tempFolderPath}\croppedVideos\upload plan 002.avi.jpg',
	        fr'{tempFolderPath}\croppedVideos\upload plan 003.avi.jpg',
	        fr'{tempFolderPath}\croppedVideos\upload plan 004.avi.jpg']
        else:
...
def addProgressbarToVideo(tempFolderPath):
    try:
...
def addTextToVideo(tempFolderPath, finalReportPath, caseName, pid, aligners):
    try:
        video=VideoFileClip(fr'{tempFolderPath}\addBackgroundVideo\final.mp4')
        clinicType = int(pid[6])
        if(clinicType != 5):
            logo = ImageClip(os.path.join(sys.path[0], fr'Resources\Images\32watts_small.png')).set_position("top").margin(top=15, left=1520, opacity=0).set_duration(82.5).set_start(5)
        else:
            logo = ImageClip(os.path.join(sys.path[0], fr'Resources\Images\lms_small.png')).set_position("top").margin(left=1450, opacity=0).set_duration(82.5).set_start(5)
        ...
        patientName = patientName.set_pos(("left","bottom")).margin(bottom=55, left=20, opacity=0).set_duration(82.5).set_start(5)
        caseId = TextClip(fr'Case Id: {pid}', fontsize = 34, color = 'black') 
        caseId = caseId.set_pos(("left","bottom")).margin(bottom=10, left=20, opacity=0).set_duration(82.5).set_start(5) 
        pauseVideo = TextClip(fr'Click to pause', fontsize = 34, color = 'black') 
        pauseVideo = pauseVideo.set_pos(("center","bottom")).margin(bottom=35, opacity=0).set_duration(82.5).set_start(5)
        imgText1 = createText('Front View', 5, 5)
        imgText2 = createText('Right View', 5, 10)
        imgText3 = createText('Left View', 5, 15)
        imgText4 = createText('Upper View', 5, 20)
        imgText5 = createText('Lower View', 5, 25)
        vidText1 = createText('Front Profile', 10.5, 30)
        vidText2 = createText('Right Profile', 10.5, 40.5)
        vidText3 = createText('Left Profile', 10.5, 51)
        vidText4 = createText('Upper Arch', 10.5, 61.5)
        vidText5 = createText('Lower Arch', 10.5, 72)
        imgText6 = createText(fr'IPR/ATT Details : {aligners}', 5, 82.5)
        video = CompositeVideoClip([video, logo, circle1, circle2, patientName, caseId, pauseVideo, imgText1, imgText2, imgText3, imgText4, 
        imgText5, vidText1, vidText2, vidText3, vidText4, vidText5, imgText6]) 
