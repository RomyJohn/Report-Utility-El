...
def createVideoThumbnail(tempFolderPath, caseName, pid, aligners, operator, logoRequired):
    try:
...
def addTextToVideo(tempFolderPath, finalReportPath, caseName, pid, aligners, logoRequired):
    try:
        video=VideoFileClip(fr'{tempFolderPath}\addBackgroundVideo\final.mp4')
        clinicType = int(pid[6])
        if(clinicType != 5 and (logoRequired=='Y' or logoRequired=='y')):
            logo = ImageClip(os.path.join(sys.path[0], fr'Resources\Images\32watts_small.png')).set_position("top").margin(top=15, left=1520, opacity=0).set_duration(62.5).set_start(5)
        elif(clinicType == 5 and (logoRequired=='Y' or logoRequired=='y')):
